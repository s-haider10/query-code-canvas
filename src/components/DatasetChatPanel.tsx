...
const DatasetChatPanel = ({
  ...
  const canSend = !!chatId && !!input.trim() && !sending && !aiThinking && !deferDisplayAI && (isFirstMessage || messages && messages.length > 0 && messages[messages.length - 1].role === "assistant");
  ...
  return <div className="flex flex-col h-full w-full max-h-full">
      {/* Messages */}
      <div className="flex-1 flex flex-col min-h-0">
        <ScrollArea className="flex-1 h-[350px] sm:h-[450px] md:h-[550px] lg:h-[650px] w-full bg-zinc-900 rounded-2xl p-3 mb-2 overflow-y-auto shadow-sm border border-zinc-800/70 transition-all" ref={scrollAreaRef}>
          <div className="flex flex-col gap-4 pb-2">
            {messagesLoading ? <Loader2 className="animate-spin w-6 h-6 m-auto text-zinc-600" /> : !chatId ? <div className="text-center text-zinc-400 mt-8 text-sm">
                Start or select a chat session to begin.
              </div> : <>
                {messages && messages.map((msg: ChatMessage, idx: number) => {
              const isUser = msg.role === "user";
              const isLastAI = msg.role === "assistant" && msg.content === latestAIContent && idx === messages.length - 1;
              if (isLastAI && deferDisplayAI && !aiError) return null;
              return <div key={msg.id} className={`flex w-full transition-all ${isUser ? "justify-end" : "justify-start"}`}>
                        <div className={`relative max-w-[78%] px-5 py-3 rounded-[2rem] text-base whitespace-pre-line shadow-lg
                            ${isUser ? "bg-[#4d818a] text-white mr-1" : "bg-zinc-800/90 text-zinc-100 border border-zinc-700/60 ml-1"}
                            animate-fade-in
                          `} style={{
                  borderRadius: "2.2rem",
                  boxShadow: isUser ? "0 2px 14px 0 rgba(77,129,138,0.17)" : "0 2px 14px 0 rgba(0,0,0,0.13)"
                }}>
                          {msg.content}
                        </div>
                      </div>;
            })}
                {(aiThinking || deferDisplayAI) && <div className="flex justify-start">
                ...
                {aiError && <div className="flex justify-start">
      ...
      }} className="flex gap-3 bg-zinc-800">
          <Input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} placeholder="Ask a question about this dataset..." disabled={false} className="rounded-full border-zinc-700 focus:border-[#4d818a] bg-zinc-900/90 px-4 py-2 text-base transition-shadow focus:ring-[#4d818a]/30 shadow-inner" autoFocus />
          <Button type="submit" disabled={!canSend} className="rounded-full px-6 py-2 bg-[#4d818a] text-white shadow-lg hover:bg-[#30636a] font-semibold transition-all">
            {aiThinking || deferDisplayAI ? <Loader2 className="animate-spin w-4 h-4" /> : "Send"}
          </Button>
        </form>
      </div>
    </div>;
